"""官方 PDF/文档/字幕等原始文件获取器（L1，二进制以可逆 Base64 保存）。"""
...
class OfficialDocumentL1Collector(BaseCollector):
    ...
    def collect(self) -> RawRecord:
        ...
        helper = RequestHelper(
            timeout=float(http.get("timeout", 60)), retries=int(http.get("retries", 3)),
        ...
        conditional = (
            {} if self.task.get("force_refresh")
            else store.request_headers(self.task["source_key"], requested_url)
        )
        ...
        # Base64 只是可逆载体编码，不解析、不清洗文件；解码后逐字节等于 HTTP 响应体。
